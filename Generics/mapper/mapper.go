...
import (
	"flag"
	"fmt"
	"go-datastructures/Generics/mapper/reflecthelper"
	"reflect"
		...
		if sourceField.Kind() != reflect.Invalid {
			...
			if !sourceField.CanInterface() {
				if mapperConfig.MapUnexportedFields {
					sourceFiledValue = reflecthelper.GetFieldValue(sourceField)
				} else {
					// for getting pointer for non-pointer struct we can use reflect.Addr() for calling pointer receivers properties
					sourceFiledValue = reflecthelper.GetFieldValueFromMethodAndReflectValue(src.Addr(), strcase.ToCamel(keys[SrcKeyIndex]))
				}
			} else {
				if mapperConfig.MapUnexportedFields {
					sourceFiledValue = reflecthelper.GetFieldValue(sourceField)
...
func processValues[TDes any, TSrc any](src reflect.Value, dest reflect.Value) error {
	...
	if src.Type() == dest.Type() {
		reflecthelper.SetFieldValue(dest, src.Interface())
		//dest.Set(src)
		return nil
	}
